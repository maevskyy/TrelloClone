...
const InfoModal = ({ visible, setVisible, count }) => {
  const [randomTip, setRandomTip] = useState({
    ...
    link: dataInfo[2].link,
  });

  const handlerRandomTip = () => {
    const { id, img, text, link } =
      dataInfo[Math.floor(Math.random() * dataInfo.length)];
      setRandomTip((prevEl) => ({
        id,
        img,
        text,
    ...
    >
      ...
      >
        <div className="flex flex-col gap-4 p-5 text-center items-center">
          <a
            href={randomTip.link}
            className="flex flex-col bg-transparent hover:cursor-pointer items-center hover:bg-slate-200 rounded-md"
          >
            <img
              src={randomTip.img}
              alt=""
          ...
          <div
            onClick={handlerRandomTip}
            className="underline text-sm pb-1 text-gray-500 hover:cursor-pointer hover:text-gray-600"
          >
            New tip.
          </div>
          <div className="border-b w-full border-gray-400"></div>
          <div className="flex flex-wrap-reverse gap-2 w-[200px] justify-center">
            <div className="underline text-sm pb-1  bg-transaprent rounded-sm p-1.5  text-gray-500 hover:cursor-pointer hover:no-underline hover:bg-gray-200">
              Applications
            </div>
            <div className="underline text-sm pb-1  bg-transaprent rounded-sm p-1.5 text-gray-500 hover:cursor-pointer hover:no-underline hover:bg-gray-200">
              Help
